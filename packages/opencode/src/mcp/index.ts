...
import { DEFAULT_MCPS } from "./defaults"
...
function configuredMcpConfig(config: Record<string, McpEntry>): Record<string, ConfigMCPV1.Info> {
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("MCP.state")(function* () {
        ...
        const config = { ...DEFAULT_MCPS, ...(cfg.mcp ?? {}) }
        const s: State = {
          config: configuredMcpConfig(config),
          status: {},
          clients: {},
          defs: {},
        }
